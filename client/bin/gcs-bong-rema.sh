...
docker run -it --rm \
  ...
  -e BONG_source.csv.files="$SOURCE_CSV_FILES" \
  -e BONG_source.year="$BONG_REMA_YEAR" \
  -e BONG_source.month="$BONG_REMA_MONTH" \
  ...
  -v "$LOCAL_CONF_FOLDER_MOUNT":/conf:Z \
  -v "$LOCAL_SOURCE_FOLDER_MOUNT":/source:Z \
